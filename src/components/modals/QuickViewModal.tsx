...
export default function QuickViewModal({ item, onClose, onAddToCart }: Props) {
  ...
  const sizeBtns = (['small', 'regular', 'large'] as const).map((sz) => {
    ...
    return (
      <button key={sz} onClick={() => setSelectedSize(sz)}
        className={'flex-1 py-2.5 border-2 border-[#C41E3A] rounded-xl font-black text-xs uppercase tracking-wider cursor-pointer transition-all ' +
          (active ? 'bg-[#C41E3A] text-white shadow-[2px_2px_0px_0px_#FFB81C]' : 'bg-white hover:bg-[#C41E3A]/5 text-[#C41E3A]')}>
        {sz} (Rs. {item?.prices?.[sz]})
      </button>
    );
  });

  const sauceBtns = sauceOptions.map((s) => {
    const active = sauce === s;
    return (
      <button key={s} onClick={() => setSauce(s)}
        className={'px-2 py-2 rounded-xl text-[9px] font-black uppercase tracking-wider text-center border-2 cursor-pointer transition-all ' +
          (active ? 'bg-[#C41E3A] text-white border-[#C41E3A]' : 'bg-white border-[#C41E3A]/10 hover:border-[#C41E3A]/30 text-[#C41E3A]/80')}>
        {s}
      </button>
    );
  });

  const drinkBtns = drinkOptions.map((d) => {
    const active = drink === d.name;
    return (
      <button key={d.name} onClick={() => setDrink(active ? '' : d.name)}
        className={'px-2 py-2 rounded-xl text-[9px] font-black uppercase tracking-wider text-center border-2 cursor-pointer transition-all ' +
          (active ? 'bg-[#FFB81C]/20 border-[#C41E3A] text-[#C41E3A]' : 'bg-white border-[#C41E3A]/10 hover:border-[#C41E3A]/30 text-[#C41E3A]/60')}>
        {d.name}
          ...
          >
            ...
            <div className="p-8 space-y-5 text-left">
              ...
              <div className="space-y-5 pt-2 border-t border-[#C41E3A]/10">
                ...
                {extraCheesePrice > 0 && (
                  <div className="space-y-2">
                    <label className="font-black uppercase text-[10px] tracking-widest text-[#C41E3A]/50 block">Extras</label>
                    <button onClick={() => setExtraCheese(!extraCheese)}
                      className={'w-full flex items-center justify-between px-4 py-3 rounded-xl border-2 cursor-pointer transition-all ' +
                        (extraCheese ? 'bg-[#FFB81C]/15 border-[#C41E3A]' : 'bg-white border-[#C41E3A]/15 hover:border-[#C41E3A]/30')}>
                      <div className="flex items-center gap-3">
                        <Sparkles className={'w-5 h-5 ' + (extraCheese ? 'text-[#FFB81C]' : 'text-[#C41E3A]/40')} />
                        <span className="font-black text-xs uppercase tracking-wider text-[#C41E3A]">Extra Cheese</span>
                      </div>
                      <div className="flex items-center gap-2">
                        <span className="text-xs font-black text-[#C41E3A]/60">+ Rs. {extraCheesePrice}</span>
                        <div className={'w-5 h-5 rounded border-2 flex items-center justify-center transition-all ' +
                          (extraCheese ? 'bg-[#C41E3A] border-[#C41E3A]' : 'border-[#C41E3A]/30')}>
                ...
                {/* Sauce Selection - only if sauces exist */}
                {sauceOptions.length > 0 && (
                  <div className="space-y-2">
                    <label className="flex items-center gap-1.5 font-black uppercase text-[10px] tracking-widest text-[#C41E3A]/50 block">
                      <Droplets className="w-3.5 h-3.5" /> Sauce
                    </label>
                    <div className="grid grid-cols-3 gap-2">{sauceBtns}</div>
                  </div>
                )}

                {/* Drink Selection - only if drinks exist */}
                {drinkOptions.length > 0 && (
                  <div className="space-y-2">
                    <label className="flex items-center gap-1.5 font-black uppercase text-[10px] tracking-widest text-[#C41E3A]/50 block">
                      <Wine className="w-3.5 h-3.5" /> Cold Drink <span className="text-[#C41E3A]/30 normal-case text-[8px]">(optional)</span>
                    </label>
                    <div className="grid grid-cols-3 gap-2">{drinkBtns}</div>
                  </div>
                )}
